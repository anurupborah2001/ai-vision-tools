...
class ReIDExtractor(AIVisionComponent):
    ...
    def setup(self, config: dict):
        if self.method == "hog":
            ...
            try:
                ...
                raise ImportError("Install with: pip install transformers torch") from e
        super().setup(config)
    ...
    def _embed(self, frame: np.ndarray, bbox: dict) -> np.ndarray:
        x1, y1, x2, y2 = (
            max(0, int(bbox["x1"])),
            max(0, int(bbox["y1"])),
            min(frame.shape[1], int(bbox["x2"])),
            min(frame.shape[0], int(bbox["y2"])),
        )
        ...
        if self.method == "hog":
            ...
            tensor = (
                cv2.cvtColor(roi_r, cv2.COLOR_BGR2RGB).astype(np.float32)[np.newaxis]
                / 255.0
            )
            ...
            inputs = self._clip_proc(
                images=Image.fromarray(roi_rgb), return_tensors="pt"
            )
            with torch.no_grad():
    ...
    def _execute(self, data, config):
        ...
        sources = (
            data.get("tracks", data.get("bboxes", [])) if isinstance(data, dict) else []
        )
        embeddings = []
        for item in sources:
