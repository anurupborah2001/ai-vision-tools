...
class TrackManager:
    ...
    def _iou_matrix(track_boxes: np.ndarray, det_boxes: np.ndarray) -> np.ndarray:
        ...
        tx1, ty1, tx2, ty2 = track_boxes[:, 0], track_boxes[:, 1], track_boxes[:, 2], track_boxes[:, 3]
    ...
    def update(self, detections: list[dict]) -> list[dict]:
        ...
        det_boxes = np.array([[d["x1"], d["y1"], d["x2"], d["y2"]] for d in detections], dtype=np.float64) \
            if detections else np.zeros((0, 4))

        track_boxes = np.array([self._kf.to_xyxy(t["mean"]) for t in self.tracks], dtype=np.float64) \
            if self.tracks else np.zeros((0, 4))

        iou_mat = self._iou_matrix(track_boxes, det_boxes)
        matched_t, matched_d = self._hungarian(iou_mat) if iou_mat.size else (np.array([]), np.array([]))

        valid_pairs = [(int(r), int(c)) for r, c in zip(matched_t, matched_d) if iou_mat[r, c] >= self.iou_threshold]
        ...
        for r, c in valid_pairs:
            det = detections[c]
            bbox = np.array([det["x1"], det["y1"], det["x2"], det["y2"]], dtype=np.float64)
            self.tracks[r]["mean"], self.tracks[r]["cov"] = self._kf.update(self.tracks[r]["mean"], self.tracks[r]["cov"], bbox)
        ...
        for j, det in enumerate(detections):
            if j not in matched_det_ids:
                ...
                self.tracks.append({
                    "track_id": self._next_id, "mean": mean, "cov": cov,
                    "hits": 1, "age": 0, "state": "tentative",
                    "label": det.get("label", ""), "conf": det.get("conf", 1.0),
                })
                self._next_id += 1
