...
class ByteTracker(AIVisionComponent):
    ...
    def _assign(self, tracks, dets, iou_thresh):
        ...
        return (
            list(zip(matched_t, matched_d, strict=False)),
            list(set(unmatched_t)),
            list(set(unmatched_d)),
        )
    ...
    def _execute(self, data, config):
        ...
        bboxes = [
            b
            for b in bboxes
            if (b["x2"] - b["x1"]) * (b["y2"] - b["y1"]) >= self.min_box_area
        ]
        ...
        pairs1, unm_t1, unm_d1 = self._assign(
            self._active, high_dets, self.match_thresh
        )
        for ti, di in pairs1:
            det = high_dets[di]
            bbox = np.array([det["x1"], det["y1"], det["x2"], det["y2"]])
            self._active[ti]["mean"], self._active[ti]["cov"] = self._kf.update(
                self._active[ti]["mean"], self._active[ti]["cov"], bbox
            )
            self._active[ti].update(
                {
                    "hits": self._active[ti]["hits"] + 1,
                    "age": 0,
                    **{k: det[k] for k in ("x1", "y1", "x2", "y2", "label", "conf")},
                }
            )
        ...
        for i in unm_t2:
            ...
            if rem_tracks[i]["age"] > self.track_buffer:
                self._active = [
                    ...
                    if t["track_id"] != rem_tracks[i]["track_id"]
                ]
