...
class PrebuiltPipelines:
    ...
    def detection_pipeline(cls, model_path: str | None = None, conf_threshold: float = 0.25,
        ...
        if model_path:
            ...
            comps.append(ObjectDetector(model_path, conf_threshold=conf_threshold, iou_threshold=iou_threshold))
        if draw:
    ...
    def augmentation_pipeline(cls, profile: str = "standard"):
        ...
        if profile == "standard":
            return _make_pipeline([Flip(), Brightness(factor_range=(0.7, 1.3)), Noise(prob=0.3)])
        if profile == "heavy":
            return _make_pipeline([
                Flip(), Brightness(factor_range=(0.5, 1.5)), Noise(prob=0.5),
                PerspectiveTransform(prob=0.3), Cutout(prob=0.3),
