    ...
    def setup(self, config: dict):
        ...
        if self.backend == "mediapipe":
            try:
                ...
                raise ImportError("Install with: pip install mediapipe") from e
        ...
        if self.backend == "mediapipe":
            ...
            for r in results:
                if r.keypoints is None:
                    continue
                for kp_set, box in zip(r.keypoints.xy, r.boxes.xyxy, strict=False):
                    kps = [
                        {
                            "name": f"kp_{i}",
                            "x": float(k[0]),
