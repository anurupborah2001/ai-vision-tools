...
class SAMSegmenter(AIVisionComponent):
    ...
    def setup(self, config: dict):
        ...
        try:
            ...
            raise ImportError("Install with: pip install segment-anything") from e
        super().setup(config)

    def _execute(self, data, config):
        frame = extract_frame(data)
        float(config.get("pred_iou_thresh", self.pred_iou_thresh))
        float(config.get("stability_score_thresh", self.stability_score_thresh))
        ...
        if prompts:
            ...
            sam_masks = [
                {"mask": m, "label": "object", "conf": float(s)}
                for m, s in zip(masks_np, scores, strict=False)
            ]
        else:
