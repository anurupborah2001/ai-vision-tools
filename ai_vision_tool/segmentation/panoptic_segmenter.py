...
class PanopticSegmenter(AIVisionComponent):
    ...
    def __init__(self, model_path: str | None = None, backend: str = "onnx",
                 stuff_classes: list[str] | None = None, thing_classes: list[str] | None = None):
        super().__init__()
    ...
    def _execute(self, data, config):
        ...
        panoptic_seg = cv2.resize(panoptic_seg.astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST).astype(np.int32)

        unique_ids = np.unique(panoptic_seg)
        segments_info = []
        for seg_id in unique_ids:
            is_thing = seg_id < len(self.thing_classes)
            label = (self.thing_classes[seg_id] if is_thing and seg_id < len(self.thing_classes)
                     else self.stuff_classes[seg_id - len(self.thing_classes)]
                     if seg_id - len(self.thing_classes) < len(self.stuff_classes)
                     else f"class_{seg_id}")
            segments_info.append({"id": int(seg_id), "label": label, "is_thing": is_thing, "conf": 1.0})
        ...
        stuff_masks = [{"mask": panoptic_seg == s["id"], "label": s["label"]}
                       for s in segments_info if not s["is_thing"]]
        thing_masks = [{"mask": panoptic_seg == s["id"], "label": s["label"]}
                       for s in segments_info if s["is_thing"]]

        payload = data if isinstance(data, dict) else {"frame": frame}
        payload.update({"panoptic_seg": panoptic_seg, "segments_info": segments_info,
                         "panoptic_overlay": panoptic_overlay, "stuff_masks": stuff_masks,
