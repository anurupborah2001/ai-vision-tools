...
class SuperResolution(AIVisionComponent):
    ...
    def __init__(self, scale: int = 2, backend: str = "auto", model_path: str | None = None):
        super().__init__()
        self.scale = scale
    ...
    def setup(self, config: dict):
        ...
        if backend in ("cv2_dnn_superres", "auto") and self.model_path:
            try:
                ...
                algo = f"EDSR" if "edsr" in self.model_path.lower() else "FSRCNN"
    ...
    def _execute(self, data, config):
        ...
        if self._backend_used == "cv2_dnn_superres" and self._sr:
            ...
            tensor = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB).astype(np.float32)[np.newaxis] / 255.0
            ...
            upscaled = cv2.resize(frame, (w * scale, h * scale), interpolation=cv2.INTER_CUBIC)
