...
class Colorizer(AIVisionComponent):
        ...
        if self.method == "opencv_dnn" and self.model_path:
            try:
                self._net = cv2.dnn.readNetFromCaffe(
                    self.model_path.replace(".caffemodel", ".prototxt"), self.model_path
                )
                if self.pts_path:
                    pts = (
                        np.load(self.pts_path)
                        .transpose()
                        .reshape(2, 313, 1, 1)
                        .astype(np.float32)
                    )
                    self._net.getLayer(self._net.getLayerId("class8_ab")).blobs = [pts]
                    self._net.getLayer(self._net.getLayerId("conv8_313_rh")).blobs = [
                        np.full([1, 313], 2.606, np.float32)
                    ]
                self._method_used = "opencv_dnn"
            except Exception as e:
                print(
                    f"[Colorizer] Failed to load DNN model: {e}. Falling back to pseudo_color."
                )
            ...
            self._method_used = (
                self.method
                if self.method in ("pseudo_color", "thermal")
                else "pseudo_color"
            )
        super().setup(config)
    ...
    def _execute(self, data, config):
        ...
        is_gray = frame.ndim == 2 or (
            frame.ndim == 3 and np.allclose(frame[:, :, 0], frame[:, :, 1])
        )
        ...
        if method == "opencv_dnn" and self._net:
            ...
            lab = cv2.cvtColor(
                cv2.cvtColor(gray_rs, cv2.COLOR_GRAY2BGR), cv2.COLOR_BGR2LAB
            )
            ...
            L_full = cv2.resize(lab[:, :, 0], (frame.shape[1], frame.shape[0])).astype(
                np.float32
            )
            ...
            lab_full[:, :, 1:] = ab_rs
            result = np.clip(
                cv2.cvtColor(lab_full.astype(np.uint8), cv2.COLOR_LAB2BGR), 0, 255
            ).astype(np.uint8)
            ...
            gray_rs = (
                cv2.resize(gray, (iw, ih)).astype(np.float32)[np.newaxis, np.newaxis]
                / 255.0
            )
