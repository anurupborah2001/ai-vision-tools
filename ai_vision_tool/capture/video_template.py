...
def video_capture_template(
    ...
    key_handlers: Optional[dict[int, Callable]] = None,
  ...
  """
    ...
    Parameters:
        ...
        key_handlers (dict, optional): A dictionary mapping key codes (int) to functions that handle those key presses. Default = None
  ...
  while True:
      ...
      if enable_auto_recording:
          ...
          cv2.putText(frame, "REC (AUTO)", (20, 80),
      ...
      if enable_manual_recording and manual_recording:
          if manual_recorder is None:
              ...
              manual_recorder = VideoRecorder(output_format=record_format, fps=safe_fps)
              manual_recorder.start(frame.shape)

          manual_recorder.write(frame)

          cv2.putText(frame, "REC (MANUAL)", (20, 120),
      ...
      # Custom key handlers
      if key_handlers and key in key_handlers:
          try:
              key_handlers[key](frame)
          except Exception as e:
              print(f"Error executing key handler for key {chr(key)}: {e}")
