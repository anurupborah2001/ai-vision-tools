...
class ONNXModel(AIVisionComponent):
    ...
    def setup(self, config: dict):
        ...
        valid_providers = [
            p for p in self.providers if p in ort.get_available_providers()
        ]
        ...
        self._output_names = self.output_names or [
    ...
    def preprocess_frame(self, frame: np.ndarray) -> np.ndarray:
        _, _, h, w = (
            self._input_shape if len(self._input_shape) == 4 else (1, 3, 640, 640)
        )
