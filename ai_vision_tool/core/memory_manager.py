...
class MemoryManager:
    ...
    def __init__(self, pool_size: int = 10, shape: tuple = (720, 1280, 3), dtype=np.uint8):
    ...
    def acquire(self, timeout: float = 1.0) -> np.ndarray:
        try:
            ...
            raise RuntimeError("MemoryManager pool exhausted — increase pool_size or release frames faster")
...
class GPUMemoryTracker:
    ...
    def log_stats(self) -> None:
        if not self._available:
            ...
            return
        print(f"[GPUMemoryTracker] Used: {self.used_mb():.1f} MB | Free: {self.free_mb():.1f} MB")
