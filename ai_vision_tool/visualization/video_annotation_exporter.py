...
class VideoAnnotationExporter(AIVisionComponent):
    ...
    def __init__(self, output_path: str, fps: float = 30.0, codec: str = "mp4v",
                 burn_annotations: bool = True, export_json: bool = True):
        super().__init__()
    ...
    def _burn(self, frame: np.ndarray, data: dict) -> np.ndarray:
        ...
        for item in items:
            ...
            if text:
                ...
                cv2.putText(out, text, (x1 + 2, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX,
    ...
    def _execute(self, data, config):
        ...
        write_frame = self._burn(frame, data) if self.burn_annotations and isinstance(data, dict) else frame
    ...
    def finalize(self):
        ...
        if self.export_json and self._annotations:
            ...
            json_path.write_text(json.dumps({"frames": self._annotations, "fps": self.fps,
                                              "frame_count": self._frame_count}, indent=2))
